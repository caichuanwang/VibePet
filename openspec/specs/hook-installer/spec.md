...
- **Claude Code**: `~/.claude/settings.json` (JSON) hook keys `PreToolUse` / `PermissionRequest` / `Stop` / `Notification` plus the lifecycle keys `SessionStart` / `UserPromptSubmit` / `SubagentStart` / `SubagentStop` / `SessionEnd` / `StopFailure` / `PermissionDenied` / `PreCompact`.
