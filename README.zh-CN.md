...
> [!IMPORTANT]
> VibePet 仍处于早期阶段，目前以源码构建为主，尚未发布可直接下载的二进制版本。请使用 Swift Package Manager 构建。工具的 Hook 格式和用户可见行为仍可能随提交发生变化。
