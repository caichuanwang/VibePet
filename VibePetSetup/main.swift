...
/// Resolves a verified executable source. An explicit override is authoritative,
/// and a failed lookup is reported instead of passing an unchecked fallback to install.
func hookBinarySource() -> HooksBinaryLocation {
    let executableDirectory = URL(fileURLWithPath: CommandLine.arguments[0])
        .resolvingSymlinksInPath()
        .deletingLastPathComponent()
    return HooksBinaryLocator.locateResult(
...
switch command {
case "install":
    guard let selectedTools = SetupToolSelector.tools(for: toolArgument) else {
        printUsage()
        break
    }
    switch hookBinarySource() {
    ...
    case .invalidExplicitOverride(let url):
        out("install failed — \(HooksBinaryLocator.environmentKey) is not executable: \(url.path)")
    case .notFound(let attempted):
        out("install failed — VibePetHooks not found; attempted: \(attempted.map(\.path).joined(separator: ", "))")
    }
